...
在前面的[《进程和线程》](../day01-15/day13/jin-cheng-he-xian-cheng.md)一文中，我们已经对在Python中使用多进程和多线程实现并发编程进行了简明的讲解，在此我们补充几个知识点。

#### threading.local类

使用线程时最不愿意遇到的情况就是多个线程竞争资源，在这种情况下为了保证资源状态的正确性，我们可能需要对资源进行加锁保护的处理，这一方面会导致程序失去并发性，另外如果多个线程竞争多个资源时，还有可能因为加锁方式的不当导致[死锁](https://zh.wikipedia.org/wiki/死锁)。要解决多个线程竞争资源的问题，其中一个方案就是让每个线程都持有资源的副本（拷贝），这样每个线程可以操作自己所持有的资源，从而规避对资源的竞争。
...
def countdown\_gen\(n, consumer\): consumer.send\(None\) while n &gt; 0: consumer.send\(n\) n -= 1 consumer.send\(None\)

## 协程 - 数据消费者

def countdown\_con\(\): while True: n = yield if n: print\(f'Countdown {n}'\) sleep\(1\) else: print\('Countdown Over!'\)

def main\(\): countdown\_gen\(5, countdown\_con\(\)\)
...
def main\(\): loop = asyncio.get\_event\_loop\(\) tasks = \[ countdown\("A", 10\), countdown\("B", 5\), \] loop.run\_until\_complete\(asyncio.wait\(tasks\)\) loop.close\(\)
